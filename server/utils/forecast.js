...
const FORECAST = (function () {
  ...
  function Day () {
    ...
    this.wind = {
      speed: {
        avg: function () {
          let total = 0
          for (let i = 0; i < this.wind.speed.timed.length; i++) {
            total += this.wind.speed.timed[i]
          }
          return total / this.wind.speed.timed.length
        },
        timed: []
      },
      direction: {
  ...
  Day.prototype.init = function init (data) {
    ...
    if (data.rain) {
      ...
      this.precipitation.timed.push(0)
    }
  }
  ...
  function parseData (data) {
    ...
    if (!data || !data.list || data.list.constructor !== Array) {
      console.error('FORECAST no data received')
      return []
    }
